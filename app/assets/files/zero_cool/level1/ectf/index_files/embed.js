...
(function() {
    ...
    var build = 'worldwide',  // emp build
        revision = '749603_749269_749444_6',  // revision of emp to use
        flashVersion = '10.2',  // version of flash to enforce
        ...
        mediator = {
            'href' : '{protocol}://{host}/mediaselector/5/select/version/2.0/mediaset/{mediaset}/transferformat/plain/vpid/{id}',  // the mediator href to use by default (unless one is set in the playlist or config xml)
            'protocol' : 'http',  // the protocol to substitute into the mediator href
            'host' : 'open.live.bbc.co.uk',  // the host (and port) to substitute into the mediator href
            'mediaset' : 'journalism-pc',  // default mediaset to substitute into the mediator href
            'backend' : ''  // the metadata service to use - 'mad' or 'pips' (appended to the mediator href e.g. '?backend=pips')
        },
            ...
            if(scriptSrc) {
                ...
                if(scriptUrl.match(scriptUrlRegex)) {
                    ...
                    for(var a = 0; a < params.length; a++) {
                        ...
                        if(param[0] == 'logLevel') {
                            logLevel = decodeURIComponent(param[1] || '');
                        }
                        else if(param[0] == 'mediaset') {
                            mediator.mediaset = decodeURIComponent(param[1] || '');
                        }
                    }
                }
            }
    ...
    define(['jquery-1', 'swfobject-2'], function ($, swf) {
        ...
        embeddedMedia.Player.prototype = {
            ...
            writeEmp: function() {
                // pass mediatorHref to player
                if(mediator.href) {
                    console.log('mediator', mediator);

                    var mediatorHref = mediator.href;
                    $.each(mediator, function(key, value) {
                        switch(key) {
                            case 'href':
                                // do nothing
                                break;
                            case 'protocol':
                                mediatorHref = mediatorHref.replace('{' + key + '}', (value === 'https' ? 'https' : 'http'));
                                break;
                            case 'backend':
                                mediatorHref += (value.length ? '?backend=' + value : '');
                                break;
                            default:
                                mediatorHref = mediatorHref.replace('{' + key + '}', value);
                                break;
                        }
                    });

                    this.setFlashvar('mediatorHref', mediatorHref);
                    console.log('mediatorHref', this.getFlashvar('mediatorHref'));
                }
            ...
            setMediator: function() {
                if(arguments.length === 2) {
                    ...
                    mediator = $.extend(true, {}, mediator, arguments[0]);
                }
            },
            getMediator: function(key) {
                if(typeof key === 'undefined') {
                    return mediator;
                }
                else {
                    return (mediator[key] || null);
                }
            },
            ...
            getFlashvar: function(key) {
                return decodeURIComponent(this.vars[key]);
            },
